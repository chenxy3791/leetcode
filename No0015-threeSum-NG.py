...
class Solution:
    ...
    #     # If one element is the same as the previous one, then no need to find pair it again.
    #     # Saving time, and also avoid duplicate.
    #     ans = []
    #     for j in range(len(nums)-1):
    #         if 2*nums[j] > target:
    #             break
    ...
    #                     break
    #     return self.removeDuplicate(ans)

    def twoSum(self, nums, target):
        # If one element is the same as the previous one, then no need to find pair it again.
        # Saving time, and also avoid duplicate.
        # Use two-pointer technique, because the input array is sorted array.
        #
        ans = []
        i = 0;
        j = len(nums) - 1
    ...
    def threeSum6(self, nums):
        ...
        # 的数组中寻找nums[j]+nums[k]=-nums[i]这个问题，也就将三数之和问题转变为二数
        # 之和---（可以使用双指针）
        # Avoid remove-duplicate thoroughly       
        if len(nums) < 3:
            return []
...
if __name__ == '__main__':    
    ...
    for i in range(200):
        ...
        nums.append(n)
    print('nums length = {0}'.format(len(nums)))    
    tStart = time.time()        
    print(sln.threeSumDP(nums))
    ...
    tStart = time.time()        
    print(sln.threeSumDPWithMemo(nums))
    tElapsed = time.time() - tStart        
    print('tElapsed = ', tElapsed, ' (sec)')
    ...
    print('\nTestcase5-2...')
    tStart = time.time()        
    ans = sln.threeSumDPWithMemo(nums)
    print(len(ans), ans)
    tElapsed = time.time() - tStart        
    print('tElapsed = ', tElapsed, ' (sec)')
    ...
    print('\nTestcase5-4...')
    tStart = time.time()        
    ans = sln.threeSum3(nums)
    print(len(ans), ans)
    tElapsed = time.time() - tStart        
    print('tElapsed = ', tElapsed, ' (sec)')

    print('\nTestcase6-1...')
    nums = [-7,2,1,10,9,-10,-5,4,13,-9,-4,6,11,-12,-6,-9,-6,-9,-11,-4,10,10,-3,-1,-4,-7,-12,-15,11,5,14,11,-7,-8,6,9,-2,9,-10,-12,-15,2,10,4,5,11,10,6,-13,6,-13,12,-7,-9,-12,4,-9,13,-4,10,4,-12,6,4,-5,-10,-2,0,14,4,4,6,13,-9,-5,-5,-13,12,-14,11,3,10,8,11,-13,4,-8,-7,2,4,10,13,7,2,2,9,-1,8,-5,-10,-3,6,3,-5,12,6,-3,6,3,-2,2,14,-7,-13,10,-13,-2,-12,-4,8,-1,13,6,-9,0,-14,-15,6,9]
    print('nums length = {0}'.format(len(nums)))
    ...
    # print('len(nums) = {0}'.format(len(nums)))
    # tStart = time.time()        
    # ans = sln.threeSum4(nums)
    # print(len(ans))
    # tElapsed = time.time() - tStart        
    ...
    print('\nTestcase8...')
    nums = [0,0,0]
    print(sln.threeSum5(nums))
    print(sln.threeSum6(nums))
